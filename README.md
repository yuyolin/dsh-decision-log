...
- **装好的那一刻，它会先在电脑上新建一个空白的 `DECISIONS.md`**（放在你项目的 `.dsh/` 目录下），里面只有一个表头、一条记录都没有——这是它的"小本本"；
- **每次对话开始，AI 都会先翻开这个小本本读一遍**（内容自动注入给 AI），所以就算一条都还没记，AI 也知道"有这么个本子在记录决策"；
...
文件在**项目文件夹**里的 `.dsh/DECISIONS.md`（每个项目一份，互不串门）。**装好后第一次开对话，它就会自动生成这个空白文件**（只有表头、零条记录），你随时可以打开看：
